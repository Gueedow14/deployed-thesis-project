...
def get_campaign_latex_table_path(dfs, y_name, cat_name, exp_name, sub_exp_name, campaign):
    setting_str = "{}-{}".format(y_name, cat_name)
    data_str = "_".join([df["data"].unique()[0] for df in dfs])

    exp_str = "{}-{}".format(exp_name, sub_exp_name)

    fig_name = "{}-{}-{}-{}".format(data_str, campaign.replace(" ", "_"), exp_str, setting_str).replace("#", "-")
    fig_path = putils.get_tuning_clusters_fig_dir_path(exp_name)
...
def main(args):
    ...
    if args["campaign"] is None:
        ...
        if src_type in ["c", "clusters"]:
            exp_name = TUNNING_CLUSTERS_EXP_NAME
            prepare_data_fn = vutils.prepare_campaign_clusters_data
            prepare_agg_data_fn = vutils.aggregate_campaign_clusters_data

        elif src_type in ["g", "graphs"]:
            exp_name = TUNNING_GRAPH_EXP_NAME
            prepare_data_fn = vutils.prepare_campaign_anony_graphs_data
            prepare_agg_data_fn = vutils.aggregate_campaign_clusters_data
        else:
            raise Exception()
        ...
        data_paths = [
            putils.get_campaign_tuning_exp_data_path(
                exp_name, data_name, sample, args["campaign"], args
            ) for data_name, sample in zip(data_names, samples)
        ]
        ...
        agg_data_paths = [
            putils.get_campaign_agg_tuning_exp_data_path(
                exp_name, data_name, sample, args["campaign"],args
            ) for data_name, sample in zip(data_names, samples)
        ]

        logger.info("\n\n\n\n")
        logger.info(agg_data_paths)
        logger.info("\n\n\n\n")
        ...
        for exp_type in exp_types:
            if exp_type == "vac":
                run_generate_campaign_gen_latex_table(agg_dfs, metric_names, args["campaign"])
            elif exp_type == "ms":
                run_generate_campaign_max_dist_latex_table(agg_dfs, metric_names, args["campaign"])
            elif exp_type == "compare":
                run_generate_campaign_comparision(agg_dfs, metric_names, args["campaign"])
            else:
                raise Exception("Unsupported exp type: {}".format(exp_types))
...
def run_generate_comparision(agg_dfs, metric_names):
    for metric_name in metric_names:
        generate_comparision(dfs=agg_dfs, y_name=metric_name, gen_str="zipf#5,50,5,2")

def run_generate_campaign_comparision(agg_dfs, metric_names, campaign):
    for metric_name in metric_names:
        generate_campaign_comparision(dfs=agg_dfs, y_name=metric_name, campaign=campaign)
...
def generate_campaign_comparision(dfs, y_name, campaign):
    cat_name = ALGO_KEY_COL
    x_name = "max_dist"

    path = get_campaign_latex_table_path(dfs, y_name, cat_name, "compare", "enforcer", campaign)
    ...
    with open(path, 'w') as f:
        ...
        for idf, new_df in enumerate(new_dfs):
            ...
            for idx, algo_name in enumerate(algo_names):
                ...
                logger.debug(current_df[["calgo_str", y_name]])
...
def generate_campaign_gen_settings_latex_table(dfs, y_name, campaign):
    cat_name = "calgo_str"

    path = get_campaign_latex_table_path(dfs, y_name, cat_name, "vac", "calgo", campaign)
    if not os.path.exists(os.path.dirname(path)):
        os.makedirs(os.path.dirname(path))
    ...
    \\footnotesize
        ...
        for idf, new_df in enumerate(new_dfs):
            ...
            hmin_val = get_value(new_df, "calgo_str", "hdbscan#min", y_name)
            hmean_val = get_value(new_df, "calgo_str", "hdbscan#mean", y_name)
            hmax_val = get_value(new_df, "calgo_str", "hdbscan#max", y_name)
            kmin_val = get_value(new_df, "calgo_str", "km#min", y_name)
            kmean_val = get_value(new_df, "calgo_str", "km#mean", y_name)
            kmax_val = get_value(new_df, "calgo_str", "km#max", y_name)
            vac_val = get_value(new_df, "calgo_str", "vac", y_name)
            # raise Exception(hmin_val)
            logger.debug(vac_val)

            line = "{first_col} & {hmin:.3f} & {hmean:.3f} & {hmax:.3f} & {kmin:.3f} & {kmean:.3f} & {kmax:.3f} & {vac:.4f} \\\\".format(
                first_col=first_col,
                hmin=hmin_val,
                hmean=hmean_val,
                hmax=hmax_val,
...
def generate_campaign_max_dist_latex_table(dfs, y_name, campaign):
    cat_name = "calgo_str"
    # x_name = "max_dist"

    path = get_campaign_latex_table_path(dfs, y_name, cat_name, "ms", "enforcer", campaign)
...
def run_generate_campaign_max_dist_latex_table(agg_dfs, metric_names, campaign):
    for metric_name in metric_names:
        generate_campaign_max_dist_latex_table(agg_dfs, metric_name, campaign)

def run_generate_campaign_gen_latex_table(agg_dfs, metric_names, campaign):
    for metric_name in metric_names:    
        generate_campaign_gen_settings_latex_table(dfs=agg_dfs, y_name=metric_name, campaign=campaign)
