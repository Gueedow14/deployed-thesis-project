...
def get_campaign_output_path(data_name, sample, campaign):
    return os.path.join(
        settings.OUTPUT_DATA_PATH, os.path.join(get_raw_graph_dir_str(data_name, sample), campaign)
    )
...
def get_campaign_raw_graph_path(campaign, data_name, sample):
    output_dir = os.path.join(os.path.join(get_output_path(data_name, sample), campaign), "raw")
    return output_dir

def get_entity_index_path(data_name, sample):
    return os.path.join(get_raw_graph_path(data_name, sample), "entities.idx")

def get_campaign_entity_index_path(campaign, data_name, sample):
    return os.path.join(get_campaign_raw_graph_path(campaign, data_name, sample), "entities.idx")
...
def get_campaign_distance_matrix_dir_path(data_name, sample, campaign):
    output_path = os.path.join(get_output_path(data_name, sample), campaign)
    pair_dist_path = os.path.join(output_path, "dist_matrix")
...
def get_campaign_distance_matrix_path(data_name, sample, info_loss_name, campaign, args):
    dir_path = get_campaign_distance_matrix_dir_path(data_name, sample, campaign)
    info_loss_str = get_info_loss_full_string(info_loss_name, args)
...
def get_campaign_k_values_dir_path(data_name, sample, campaign):
    output_path = get_output_path(data_name, sample)
    return os.path.join(output_path, os.path.join(campaign, "k_values"))
...
def get_campaign_k_values_path(data_name, sample, campaign, args):
    dir_path = get_campaign_k_values_dir_path(data_name, sample, campaign)
    return os.path.join(dir_path, campaign.replace(' ', '_') + ".txt")

def get_clusters_dir_path(data_name, sample, anony_mode):
    output_path = get_output_path(data_name, sample)
    return os.path.join(output_path, "clusters", anony_mode)

def get_campaign_clusters_dir_path(data_name, sample, anony_mode, campaign):
    output_path = get_campaign_output_path(data_name, sample, campaign)
    return os.path.join(output_path, "clusters", anony_mode)
...
def get_campaign_anony_graphs_dir_path(data_name, sample, campaign):
    output_path = get_campaign_output_path(data_name, sample, campaign)
    return os.path.join(output_path, "graphs")
...
def get_campaign_raw_clusters_str(campaign, info_loss_name, handler_name, calgo_name, args):
    campaign_str = campaign.replace(" ", "_")
    ifn_str = get_info_loss_full_string(info_loss_name, args)
    calgo_str = get_clustering_str(calgo_name, args)
    handler_str = get_outliers_handler_str(handler_name, args)
    return "{camp}_{ifn}_{oh}_{calgo}".format(camp=campaign_str, ifn=ifn_str, oh=handler_str, calgo=calgo_str)
...
def get_campaign_raw_clusters_path(data_name, sample, campaign, info_loss_name, handler_name, calgo_name, args):
    raw_dir_path = get_campaign_clusters_dir_path(data_name, sample, "raw", campaign)
    raw_clusters_name = get_campaign_raw_clusters_str(campaign.replace(" ", "_"), info_loss_name, handler_name, calgo_name, args)
...
def get_campaign_anony_clusters_str(campaign, info_loss_name, handler_name, calgo_name, enforcer_name, args):
    raw_cluster_str = get_campaign_raw_clusters_str(campaign, info_loss_name, handler_name, calgo_name, args)
    enforcer_str = get_enforcer_name(enforcer_name, args)
...
def get_campaign_anony_clusters_path(data_name, sample, campaign, info_loss_name, handler_name, calgo_name, enforcer_name, args):
    anony_dir_path = get_campaign_clusters_dir_path(data_name, sample, "anony", campaign)
    anony_clusters_name = get_campaign_anony_clusters_str(campaign, info_loss_name, handler_name, calgo_name, enforcer_name, args)
...
def get_campaign_anony_graph_path(data_name, sample, campaign, info_loss_name, handler_name, calgo_name, enforcer_name, args):
    anony_dir_path = get_campaign_anony_graphs_dir_path(data_name, sample, campaign)
    anony_clusters_name = get_campaign_anony_clusters_str(campaign.replace(" ","_"), info_loss_name, handler_name, calgo_name, enforcer_name, args)

    return os.path.join(anony_dir_path, anony_clusters_name)


def get_campaign_anony_graph_file_path(data_name, sample, campaign, file):
    path = get_campaign_output_path(data_name, sample, campaign)

    return os.path.join(path, "raw", file)
...
def get_campaign_tuning_exp_data_path(exp_name, data_name, sample, campaign, args):
    data_str = get_raw_graph_dir_str(data_name, sample)

    return os.path.join(settings.EXP_DATA_PATH, exp_name, data_str, "{}.csv".format(campaign.replace(" ", "_")))
...
def get_campaign_agg_tuning_exp_data_path(exp_name, data_name, sample, campaign, args):
    data_str = get_raw_graph_dir_str(data_name, sample)

    return os.path.join(settings.EXP_DATA_PATH, exp_name, data_str, "{}_agg.csv".format(campaign.replace(" ", "_")))
...
def extract_info_from_campaign_data_str(data_str):
    try:
        splits = data_str.split("_")
        return {
            "data": splits[0] + "_" + splits[1],
            "sample": int(splits[2]),
        }
    except Exception as e:
        logger.error("str: {} - splits: {}".format(data_str, splits))
...
def extract_info_from_campaign_clusters_str(clusters_str):
    splits = clusters_str.split("_")

    for i, str in enumerate(splits):
        if str[0:4] == "adm#":
            post_campaign = i
            campaign = ""
            for count in range(i):
                campaign += splits[count] + "_"
    ...
    if post_campaign:
        for i, str in enumerate(splits):
            if i >= post_campaign:
                new_splits.append(str)

    campaign = campaign[:-1]

    info = extract_info_from_campaign_gen_str(campaign)

    info.update(extract_info_from_info_loss_str(new_splits[0]))
    info.update(extract_info_from_outliers_handler_str(new_splits[1]))
    info.update(extract_info_from_calgo_str(new_splits[2]))

    if len(new_splits) == 4:
        info.update(extract_info_from_enforcer_str(new_splits[3]))
...
def extract_info_from_campaign_gen_str(campaign):
    
    info = {
        "max_k": dutils.get_max_kval_in_campaign(campaign.replace("_", " ")),
        "min_k": dutils.get_min_kval_in_campaign(campaign.replace("_", " ")),
    }
...
def extract_info_from_anonymized_subgraph_path(graph_path):
    ...
    if splits[2].split("_")[0] != "anonykg":
        info = extract_info_from_data_str(splits[2])
        info.update(extract_info_from_clusters_str(splits[-1]))
    else:
        info = extract_info_from_campaign_data_str(splits[2])
        info.update(extract_info_from_campaign_clusters_str(splits[-1]))
