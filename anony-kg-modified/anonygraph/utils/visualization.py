...
def get_exp_data(
    ...
    if refresh or not os.path.exists(exp_path):
        ...
        if args["campaign"] is None:
            ...
            raw_data = prepare_data_fn(prepare_data_args, workers, args["campaign"], args)
...
def get_campaign_graphs_quality_from_path(graph_path, campaign):

    info = putils.extract_info_from_anonymized_subgraph_path(graph_path)
    logger.debug(info)

    anonymized_subgraph = dutils.load_anonymized_campaign_graph_from_path(
        data_name=info["data"],
        sample=info["sample"],
        campaign=campaign,
        info_loss_name=info["info_loss"],
    ...
    raw_subgraph = dutils.load_raw_campaign_graph(info["data"], campaign, info["sample"])
...
def prepare_campaign_clusters_data(data_info, num_workers, campaign, args):
    data_name, sample = data_info["data"], data_info["sample"]

    clusters_dir_path = putils.get_campaign_clusters_dir_path(data_name, sample, "anony", campaign)
    logger.debug("clusters path: {}".format(clusters_dir_path))
...
def prepare_campaign_anony_graphs_data(data_info, num_workers, campaign, args):
    data_name, sample = data_info["data"], data_info["sample"]

    anony_graphs_dir_path = putils.get_campaign_anony_graphs_dir_path(data_name, sample, campaign)
    logger.debug("anony graphs path: {}".format(anony_graphs_dir_path))
    ...
    raw_data = list(
        Parallel(n_jobs=num_workers)(
            delayed(get_campaign_graphs_quality_from_path)(path, campaign)
            for path in tqdm(anony_graphs_paths)
        )
    )
...
def aggregate_campaign_clusters_data(data_info, num_workers, campaign, args):
    df = data_info["df"]
