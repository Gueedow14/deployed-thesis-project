...
def main(args):
    ...
    if args["campaign"] is None:
        ...
        # load dist matrix
        dist_matrix = dutils.load_campaign_dist_matrix(data_name, sample, info_loss_name, args["campaign"], args)
        logger.debug("dist matrix shape: {}".format(dist_matrix.shape))

        # load k values
        entity_id2k_dict = dutils.load_campaign_entity_id2k_dict(data_name, sample, args["campaign"], args)
        logger.debug("id2k dict: {}".format(entity_id2k_dict))
        ...
        # load relative ids
        entity_id2idx_dict = dutils.load_campaign_entity_id2idx_dict(data_name, sample, args["campaign"])
        logger.debug("id2idx dict: {}".format(entity_id2idx_dict))

        logger.info(entity_id2idx_dict)
        logger.info("\n\n")

        entity_idx2id_dict = dutils.load_campaign_entity_idx2id_dict(data_name, sample, args["campaign"])
        logger.debug("idx2id dict: {}".format(entity_idx2id_dict))
        ...
        # load raw clusters
        clusters = dutils.load_campaign_raw_clusters(data_name, sample, args["campaign"], info_loss_name, handler_name, calgo_name, args)
        logger.debug("raw clusters: {}".format(clusters))
        ...
        anonymized_clusters_path = putils.get_campaign_anony_clusters_path(data_name, sample, args["campaign"], info_loss_name, handler_name, calgo_name, enforcer_name, args)
        new_clusters.to_file(anonymized_clusters_path)
        logger.info("saved to: {}".format(anonymized_clusters_path))
