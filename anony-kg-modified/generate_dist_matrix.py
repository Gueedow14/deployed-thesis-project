...
def generate_pair_distances_for_campaign_graph(data_name, sample, campaign, info_loss_name, args):
    # load subgraph
    graph = dutils.load_raw_campaign_graph(data_name, campaign, sample)
    logger.debug(graph)
    ...
    # get relative ids
    entity_idx2id = dutils.load_campaign_entity_idx2id_dict(data_name, sample, campaign)
    logger.debug(entity_idx2id)

    dist_matrix_path = putils.get_campaign_distance_matrix_path(data_name, sample, info_loss_name, campaign, args)
    logger.debug(dist_matrix_path)
    if not os.path.exists(os.path.dirname(dist_matrix_path)):
        logger.info("creating folder: {}".format(os.path.dirname(dist_matrix_path)))
        os.makedirs(os.path.dirname(dist_matrix_path))
...
def main(args):
    ...
    if args["campaign"] is None:
        ...
        generate_pair_distances_for_campaign_graph(args["data"], args["sample"], args["campaign"], args["info_loss"], args)
